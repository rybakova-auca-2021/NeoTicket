...
class CodeAdapter(private var tickets: List<DisplayableItem>) : RecyclerView.Adapter<CodeAdapter.TicketViewHolder>() {
            ...
            binding.apply {
                when (item) {
                    is MyMovieTicketItem -> {
                        ...
                        Glide.with(itemView.context).load(ticket.qr_code).into(codeImg)
