...
class ConfirmSportsTicketSeatsFragment : BottomSheetDialogFragment() {
    private lateinit var binding: FragmentConfirmTicketSeatsBinding
    private val viewModel: CreateSportTicketViewModel by viewModels()
    ...
    companion object {
        fun newInstance(showTime: Int): ConfirmSportsTicketSeatsFragment {
            val fragment = ConfirmSportsTicketSeatsFragment()
            val args = Bundle()
            ...
            if (Util.token != null) {
                viewModel.createTicketLiveData.observe(viewLifecycleOwner, Observer { result ->
                    if (result != null) {
                        ...
                        bundle.putString("source", "sportTicket")
